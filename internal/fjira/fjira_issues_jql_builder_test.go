...
func Test_buildSearchIssuesJql(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, buildSearchIssuesJql(tt.args.project, tt.args.query, tt.args.status, tt.args.user), "buildSearchIssuesJql(%v, %v, %v, %v)", tt.args.project, tt.args.query, tt.args.status, tt.args.user)
		})
	}
}
