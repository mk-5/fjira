...
func Test_readFromUserSettings(t *testing.T) {
	...
	tests := []struct {
		name string
		args args
		want *workspaces.WorkspaceSettings
	}{
		{"should Read user settings from current workspace",
			...
			&workspaces.WorkspaceSettings{JiraToken: "123", JiraUsername: "test@test.pl", JiraRestUrl: "https://test.atlassian.net", Workspace: "xyz"},
		},
		{"should Read user settings from another workspace",
			...
			&workspaces.WorkspaceSettings{JiraToken: "111", JiraUsername: "test_user", JiraRestUrl: "https://test", Workspace: "abc"},
		},
		{"should Read user settings from default workspace",
			...
			&workspaces.WorkspaceSettings{JiraToken: "333", JiraUsername: "test_user2", JiraRestUrl: "https://test2", Workspace: "default"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_readFromUserInputAndWorkspaceEdit(t *testing.T) {
	...
	type args struct {
		workspace        string
		existingSettings *workspaces.WorkspaceSettings
	}
	tests := []struct {
		name string
		args args
		...
		{"should Read workspace data from user input",
			args{workspace: "xyz", existingSettings: &workspaces.WorkspaceSettings{JiraToken: "123", JiraUsername: "test@test.pl", JiraRestUrl: "https://test.atlassian.net"}}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// given
			...
			assert2.FileExists(t, tempDir+"/.fjira/fjira.yaml")
